...
class PurchaseOperation:
    ...
    def get_purchases_by_status(self, status):
        db = None
        cursor = None
        try:
            db = self.connection()
            ...
            cursor.execute(
                """
                SELECT email, username, phone, course_code, title, subtitle, price,
            ...
            logging.exception("Error in get_purchases_by_status")
            return []
        finally:
            if cursor: cursor.close()
            if db: db.close()
    ...
    def delete_purchase_by_id(self, purchase_id):
        ...
        try:
            ...
            logging.exception(f"Error deleting purchase with ID: {purchase_id}")
            if db: db.rollback()
            return False
        finally:
            if cursor: cursor.close()
