...
def process_question_for_display(question):
    ...
    image_html = ''
    if image_path:
        image_html = f'<img src="{image_path}" class="question-inline-image" alt="Question Image">'

    if image_html:
       
        processed_text = re.sub(r'<span[^>]*>\s*\[IMAGE_PLACEHOLDER\]\s*<\/span>', image_html, processed_text, flags=re.IGNORECASE)
      
      
        processed_text = processed_text.replace('[IMAGE_PLACEHOLDER]', image_html)

    processed_text = processed_text.replace('<p>', '').replace('</p>', '').strip()
    processed_text = f'<p>{processed_text}</p>'

    question['display_question_html'] = processed_text
    return question
...
def browse_by_topic_with_code(unique_code, paper_code):
    ...
    return render_template('users/question_practice/topic_select.html', subjects=subjects, paper_code=paper_code, username=session['username'], email=email, unique_code=unique_code)
...
def browse_topic_subject(unique_code, paper_code, subject):
    ...
    return render_template('users/question_practice/topic_list.html', topics=topics, subject=subject, paper_code=paper_code, username=session['username'], email=email, unique_code=unique_code)
...
def show_topic_questions(unique_code, paper_code, subject, topic):
    ...
    questions_data = user_op.fetch_all("SELECT * FROM questions WHERE subject = %s AND topic = %s AND paper_code = %s LIMIT %s OFFSET %s", (subject, topic, paper_code, per_page, offset))
    
    # Process each question for display
    questions = [process_question_for_display(q) for q in questions_data]
    
    total_pages = (total + per_page - 1) // per_page
    return render_template('users/question_practice/display_questions.html', questions=questions, section='topic', subject=subject, topic=topic, paper_code=paper_code, page=page, total_pages=total_pages, username=session['username'], email=email, unique_code=unique_code)
...
def browse_by_subject(unique_code, paper_code):
    email = session['email']
    subjects = user_op.fetch_all("SELECT DISTINCT subject FROM questions WHERE paper_code = %s", (paper_code,))
    return render_template('users/question_practice/subject_select.html', subjects=subjects, paper_code=paper_code, username=session['username'], email=email, unique_code=unique_code)
...
def show_subject_questions(unique_code, paper_code, subject):
    ...
    offset = (page - 1) * per_page
    questions_data = user_op.fetch_all("SELECT * FROM questions WHERE subject = %s AND paper_code = %s LIMIT %s OFFSET %s", (subject, paper_code, per_page, offset))
    
    # Process each question for display
    questions = [process_question_for_display(q) for q in questions_data]
    
    total_pages = (total + per_page - 1) // per_page
    return render_template('users/question_practice/display_questions.html', questions=questions, section='subject', subject=subject, paper_code=paper_code, page=page, total_pages=total_pages, username=session['username'], email=email, unique_code=unique_code)
...
def browse_by_year(unique_code, paper_code):
    ...
    return render_template('users/question_practice/year_select.html', years=years, paper_code=paper_code, username=session['username'], email=email, unique_code=unique_code)
...
def show_year_questions(unique_code, paper_code, year):
    ...
    offset = (page - 1) * per_page
    questions_data = user_op.fetch_all("SELECT * FROM questions WHERE year = %s AND paper_code = %s LIMIT %s OFFSET %s", (year, paper_code, per_page, offset))
    
    # Process each question for display
    questions = [process_question_for_display(q) for q in questions_data]
    
    total_pages = (total + per_page - 1) // per_page
    return render_template('users/question_practice/display_questions.html', questions=questions, section='year', year=year, paper_code=paper_code, page=page, total_pages=total_pages, username=session['username'], email=email, unique_code=unique_code)
