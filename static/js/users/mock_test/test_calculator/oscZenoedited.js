...
if (inBox.val().includes("Infinity") || inBox.val().includes(strMathError)) return;

switch (this.id) {
    case 'keyPad_btnPlus':
        stackCheck($("#" + this.id).text());
        newOpCode = 1;
        if (opCode === 10 && stackArray.length > 0 && stackArray[stackArray.length - 1] === "{") opcodeChange();
        operation();
        stackVal1 = 0;
        break;
    ...
    case 'keyPad_btnMinus':
        stackCheck($("#" + this.id).text());
        newOpCode = 2;
        if (opCode === 10 && stackArray.length > 0 && stackArray[stackArray.length - 1] === "{") mopcodeChange();
        operation();
        stackVal1 = 0;
        break;
    ...
    case 'keyPad_btnMult':
        ...
        if (opCode === 10) {
            const lastOp = opCodeArray[opCodeArray.length - 1];
            const lastStack = stackArray[stackArray.length - 1];
            if (lastOp < 3 || (stackArray.length > 0 && lastStack === "{")) {
                opcodeChange();
            } else {
                operation();
            }
    ...
    case 'keyPad_btnDiv':
        ...
        if (opCode === 10) {
            const lastOp = opCodeArray[opCodeArray.length - 1];
            const lastStack = stackArray[stackArray.length - 1];
            if (lastOp < 4 || stackVal1 === 5 || (stackArray.length > 0 && lastStack === "{")) {
                opcodeChange();
            } else {
                operation();
            }
    ...
    case 'keyPad_%':
        ...
        if (opCode === 10) {
            const lastOp = opCodeArray[opCodeArray.length - 1];
            const lastStack = stackArray[stackArray.length - 1];
            if (lastOp < 6 || (stackArray.length > 0 && lastStack === "{")) {
                opcodeChange();
            } else {
                operation();
            }
    ...
    case 'keyPad_btnMod':
        ...
        if (opCode === 10) {
            const lastOp = opCodeArray[opCodeArray.length - 1];
            const lastStack = stackArray[stackArray.length - 1];
            if (lastOp === 1 || lastOp === 2 || (stackArray.length > 0 && lastStack === "{")) {
                opcodeChange();
            } else {
                operation();
            }
    ...
    case 'keyPad_btnClose':
        if (stackVal2 === 6) {
            ...
            if (stackVal1 !== 3) {
                if (inBox1.val().includes("e+0") && inBox.val().includes("-")) {
                    inBox1.val(inBox1.val().replace("e+0", "e"));
                } else if (inBox1.val().includes("e+0")) {
                    inBox1.val(inBox1.val().replace("e+0", "e+"));
                }
                displayString = inBox1.val() + inBox.val() + $("#" + this.id).text();
            } else {
...
$("a.keyPad_btnUnaryOp").click(function () {
    const inputBox = $('#keyPad_UserInput');
    const value = inputBox.val();
    const x = parseFloat(value);
    let retVal = oscError;

    // Prevent operation if invalid input is detected
    if (value.includes("Infinity") || value.includes(strMathError)) return;

    switch (this.id) {
        case 'keyPad_btnInverseSign':
            retVal = -x; trig = 1; stackVal2 = 3;
            break;
        ...
        case 'keyPad_btnLn':
            retVal = Math.log(x);
            displayTrignometric($(this).text(), x);
            break;
        ...
        case 'keyPad_btn10X':
            ...
            if (retVal % 10 !== 0 && (Math.abs(retVal) < 1e-8 || Math.abs(retVal) > 1e8) && x % 1 === 0) {
        ...
        case 'keyPad_btnAsin':
            ...
            modeText("asin", x); trig = 1;
            break;

        case 'keyPad_btnAcos':
            retVal = cosInvCalc(modeSelected, x);
            modeText("acos", x); trig = 1;
            break;

        case 'keyPad_btnAtan':
            retVal = tanInvCalc(modeSelected, x);
            modeText("atan", x); trig = 1;
            break;
        ...
        case 'keyPad_btnSinH':
            retVal = (Math.exp(x) - Math.exp(-x)) / 2;
            modeText($(this).text(), x);
            break;
        ...
        case 'keyPad_btnTgH':
            retVal = (Math.exp(x) - Math.exp(-x)) / (Math.exp(x) + Math.exp(-x));
            modeText($(this).text(), x);
            break;

        case 'keyPad_btnSecH':
            retVal = 2 / (Math.exp(x) + Math.exp(-x));
            break;
    ...
    // Update the display/output if retVal is valid
    if (retVal !== oscError) {
        inputBox.val(retVal);
    }
...
$(document).ready(function() {
    ...
    $('#keyPad_Help').click(function() {
        ...
        // Close if other help-related elements are open
        $('#keyPad_Helpback').hide();
    });

   
    $('#closeButton').click(function() {
        // For a real application, this would close the window
        // For demo purposes, we'll hide the calculator with animation
        $('#keyPad').fadeOut(300, function() {
            // Optional: Add callback after animation
    ...
    $('#calc_min').click(function() {
        ...
        $('#keyPad').toggleClass('minimized');
        
        if ($('#keyPad').hasClass('minimized')) {
            // Minimize action
            ...
            // Restore action
            $('#mainContentArea').slideDown(200);
            
            $('#keyPad').css('height', 'auto');
        }
        
        // Ensure help content closes when minimizing
        $('#helpContent').slideUp(200);
    });


    $(document).mouseup(function(e) {
        const helpContent = $('#helpContent');
        const helpButton = $('#keyPad_Help');
        
        // If click is outside help content and help button
        if (!helpContent.is(e.target) && 
            helpContent.has(e.target).length === 0 && 
