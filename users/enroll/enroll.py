...
def start_enrollment():
    try:
        ...
        logger.info(f"Captcha generated for user session: {captcha}")
        return jsonify({"captcha": captcha})
    except Exception as e:
        logger.exception("Error generating captcha for user enrollment")
...
def check_enrollment():
    ...
    try:
        ...
        logger.exception("Error fetching enrolled courses for user")
...
def final_enroll():
    try:
        logger.info("Enrollment process started for user")
        ...
        if not course_code or not course_name or not captcha_input or not captcha_value:
            logger.warning("Missing required enrollment fields for user.")
            return jsonify({"success": False, "error": "Missing fields. Please try again."}), 400

        if captcha_input != captcha_value:
            logger.warning("Captcha verification failed for user %s", email)
            session.pop('captcha_value', None)
        ...
        enrolled_courses = user_op.get_enrolled_courses(email)
        if len(enrolled_courses) >= 2:
            logger.info("Enrollment limit exceeded for user %s", email)
            session.pop('captcha_value', None)
        ...
        logger.exception("Unexpected error during final user enrollment")
...
def get_unique_code():
    ...
    try:
        unique_code = user_op.get_user_unique_code(user_email, course_code)
        if unique_code:
        ...
        logger.exception("Error retrieving unique code for user")
